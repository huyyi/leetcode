...
public:
    int strToInt(string str) {
        ...
        if (idx >= str.size() || str[idx] - '0' > 9) return 0;
        int res = 0;
        while (idx < str.size() && str[idx] <= '9' && str[idx] >= '0') {
            if (res > INT_MAX / 10 || res == INT_MAX / 10 && str[idx] > '7') {
                return m ? INT_MIN : INT_MAX;
            }
